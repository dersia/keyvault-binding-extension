...
{
    public class KeyVaultWrapKeyInputAsyncConverter : IAsyncConverter<KeyVaultWrapKeyAttribute, byte[]>
    {
        private readonly INameResolver _nameResolver;
        private readonly Microsoft.Extensions.Logging.ILogger _logger;
        private readonly IKeyVaultClient _keyVaultClient;

        public KeyVaultWrapKeyInputAsyncConverter(INameResolver nameResolver, Microsoft.Extensions.Logging.ILogger logger, IKeyVaultClient keyVaultClient)
        {
        ...
        public async Task<byte[]> ConvertAsync(KeyVaultWrapKeyAttribute config, CancellationToken cancellationToken)
        {
            ...
            var result = await _keyVaultClient.WrapKeyAsync(config.KeyVaultUrl, config.KeyName, config.KeyVersion, config.Algorithm.MapAlogrithm(), Encoding.UTF8.GetBytes(_nameResolver.Resolve(config.Value) ?? config.Value), cancellationToken);
            return result.Result;
        }

        private async Task<KeyBundle?> GetKey(KeyVaultWrapKeyAttribute config, CancellationToken cancellationToken)
        {
            KeyBundle? key = null;
            try
            {
